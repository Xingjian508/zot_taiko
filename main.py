...
def run_game_with_music():
  ...
  while not storage.signal['terminate'] and mixer.music.get_busy():
    ...
    new_left_zot = storage.signal['don'] == 'no_zot'
    new_right_zot = storage.signal['kat'] == 'no_zot'

    left_click = left_zot and not new_left_zot
    right_click = right_zot and not new_right_zot
